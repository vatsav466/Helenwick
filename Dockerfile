...
# NOTE: backend/api_manager/.alg_env is INTENTIONALLY NOT copied here.
# It contains live credentials (DB, SMTP, ThingsBoard, encryption key) and
# must only ever be provided at *runtime* via a read-only volume mount
# (see docker-compose.yml / deploy.yml), never baked into the image or
# committed to git.
